    ...
    >
      ...
      >
        <template slot-scope="scope">
           <el-button
            v-for="(item, i) in dxfOperatetypes"
            :key="i"
            size="medium"
            :style="item.background"
            style="font-size: 14px"
            :disabled = item.btnDisabled
             v-if="scope.row.subjectstatus == '待下发'"
            @click.native.prevent="doOpeartion(scope.$index, tabledata, item)"
            >{{ item.operation }}</el-button
          >
           <el-button
            v-for="(item, i) in ywcOperatetypes"
            :key="i"
            size="medium"
            :style="item.background"
            style="font-size: 14px"
            :disabled = item.btnDisabled
             v-if="scope.row.subjectstatus == '已完成'"
            @click.native.prevent="doOpeartion(scope.$index, tabledata, item)"
            >{{ item.operation }}</el-button
          >
            <el-button
            ...
            style="font-size: 14px"
            :disabled = item.btnDisabled
             v-if="scope.row.subjectstatus == '执行中'"
            @click.native.prevent="doOpeartion(scope.$index, tabledata, item)"
            >{{ item.operation }}</el-button
          >
...
export default {
  ...
  methods: {
    ...
    doOpeartion(index, rows, item) {
      ...
      this.$emit("specialOperation", index, rows, item);
    },
...
.commonTable .el-table_1_column_2 .el-tooltip{
  cursor: pointer!important;
